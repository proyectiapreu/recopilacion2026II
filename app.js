...
function buscar() {
  ...
  const row = DATA.respuestas.find(r => (r.codigo || '').toUpperCase() === code);
...
(async () => {
  try {
    ...
    getEl('estado').textContent = 'Datos cargados. Escribe un código para consultar el reporte.';
    ...
    getEl('estado').textContent = 'No se pudieron cargar los CSV. Revisa que data/preguntas.csv, data/respuestas_estudiantes.csv y data/cohorte_ficticia.csv existan en GitHub Pages.';
    console.error(e);
  }
})();

getEl('buscar').addEventListener('click', buscar);
getEl('codigo').addEventListener('keydown', e => { if (e.key === 'Enter') buscar(); });
