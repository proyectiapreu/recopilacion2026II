...
function buscar() {
  ...
  areaScores.slice().sort((a, b) => AREA_CONFIG.findIndex(x => x.key === a.area) - AREA_CONFIG.findIndex(x => x.key === b.area)).forEach(a => {
    ...
    const conf = AREA_CONFIG.find(x => x.key === a.area);
    const maxArea = conf?.max || 20;
    const pctAcierto = Math.round((a.raw / maxArea) * 100);
    areasDiv.insertAdjacentHTML('beforeend', `<div class="area-row"><div class="area-name">${a.label}<span class="area-meta">${a.raw} de ${maxArea} aciertos</span><span class="area-percent">${pctAcierto}% de acierto</span></div><div class="bar"><span style="width:${width}%"></span></div><div class="area-score">${a.score.toFixed(1)}</div></div>`);
  });
  ...
  estado.innerHTML = `Reporte generado para<br><strong>${getStudentId(row)}</strong>.`;
...
async function init() {
  ...
  try {
    ...
    estado.textContent = `Datos cargados: ${DATA.preguntas.length} preguntas, ${DATA.respuestas.length} registros y ${DATA.cohorte.length} casos de cohorte. Ingresa tu número de documento para consultar el reporte.`;
